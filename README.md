...
- `bot-settings.spec.ts` — Bot 平台設定分頁測試（兩平台欄位狀態、更換單一欄位、主動推送開關、測試連線成功與失敗、清除確認、非管理員擋下）
