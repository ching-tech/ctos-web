...
- `utils.ts` — 工具函式
- `kb.ts` — 知識庫 API 客戶端（清單／詳情／建立／編輯／刪除／附件／分享／版本歷史／重建索引）
...
- `bot.ts` — Bot 管理 API 客戶端（綁定狀態、群組、使用者含單筆明細、黑名單、訊息、檔案含群組專用端點；分頁與檔案下載 Helper）
...
- `kb/list.tsx` — 知識庫清單頁（搜尋、scope／type／category 篩選、URL 同步；管理員多一個「重建索引」）
...
- `bot/group-detail.tsx` — Bot 群組明細頁（資訊、最近訊息、刪除；兩個分頁籤「總覽」「檔案」，tab 寫進網址）
...
- `bot/tabs/group-files.tsx` — 群組明細的檔案分頁（打 `GET /api/bot/groups/{id}/files`，只有類型篩選與分頁）
...
- `bot/file-table.tsx` — Bot 檔案表格（檔案分頁與群組明細的檔案分頁共用列、預覽、下載、刪除；`showSource` 控制「來源」欄）
...
- `kb-list.spec.ts` — 知識庫清單測試（含管理員重建索引與非管理員看不到入口）
...
- `bot-group-detail.spec.ts` — Bot 群組明細測試（含檔案分頁：專用端點、tab 寫進網址、類型篩選）
...
- **知識庫** — 路由 `/kb`，清單搜尋、閱讀附件、新增編輯、刪除、分享連結、版本歷史；首頁多「最近更新」。清單頁頂部管理員多一個「重建索引」（`POST /api/knowledge/rebuild-index`，`api/knowledge.py` 188–205），確認對話框照 PR #29 的做法，進行中停用按鈕。後端這支要的其實只是 `knowledge-base` 的 app 權限（不是 admin），也就是能進這頁的人都呼叫得動；前端刻意收得比後端緊，只給管理員看到入口。回應是沒有 `response_model` 的裸 dict `{total, errors, next_id}`（`services/knowledge.py` 819–823），前端把它組成「已重建索引，共 N 筆，下一個編號 M」，`errors` 非空時把每一筆讀不進去的檔案接在後面；失敗時 500 的 `detail` 原樣顯示
...
- **訊息中心** — 路由 `/messages`，只要登入就進得去（後端 `api/messages.py` 只掛 `get_current_session`，沒有 app 閘）。Header 右側的鈴鐺顯示未讀數（超過 99 顯示 `99+`），點了帶著 `?is_read=false` 進未讀清單。清單有嚴重程度與來源兩個多選（同名參數重複帶，後端收 `list[...]`）、已讀狀態、關鍵字與日期區間，全部寫進網址；表格列嚴重程度、來源、分類、標題、時間與已讀狀態，未讀列加粗，手機寬度改卡片。可勾選多筆「標為已讀」，也可以「全部標為已讀」（有確認對話框，不受目前篩選影響）。`/messages/:id` 明細顯示標題、內容（純文字保留換行）、可摺疊的附加資料 `<pre>`、時間、嚴重程度與來源；後端讀明細**不會**順手標已讀，所以進頁面後由前端補送一次 `mark-read {ids:[id]}`。鈴鐺的未讀數用 React Query 輪詢（每分鐘一次加視窗重新取得焦點），沒有接 socket——後端雖然有 `message:unread_count` 事件，但 ctos-web 目前只在 AI 助手頁連 socket，要即時再改接
  ...
  **依使用者篩選只有管理員看得到**：篩選列多一個「使用者」下拉（選項來自 `GET /api/user/list`），選了寫進網址 `?user_id=`；非管理員完全不顯示這個下拉。後端對非管理員送的 `user_id` 是**直接忽略**（`filter.user_id = user_id if is_admin else None`），不是回 4xx。管理員選了人之後，清單的 `user_id` 是嚴格相等（`user_id = $1`），**不含**全系統訊息（`user_id IS NULL`）——跟非管理員預設看到的「自己的 + 全系統」語意不一樣。「全部標為已讀」也會帶著這個篩選，但走的是查詢字串 `POST /api/messages/mark-read?user_id=`（不在請求體裡，只有搭配 `all: true` 才生效，`services/message.py` 268–290），而且它的 SQL 是 `user_id = $1 OR user_id IS NULL`，所以標記範圍其實是「所選使用者 + 全系統」，比清單篩選寬——確認對話框文字有把這點寫清楚。單筆勾選標已讀是照 `ids` 標記，`user_id` 對它完全沒作用。
